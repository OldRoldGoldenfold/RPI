...
public class KeyValuePair implements Comparable<KeyValuePair> {
	private int key;		// 8 digits
	private String value;	// 8 characters
	 ...
	 * KeyValuePair object to 8-digit <code>int</code> and 8-character <code>String</code>
	 */
	public void makeRandom()	{
		...
		int place = 1;
		for(int i = 0; i < 8; i++)	{
			key += ((int)(Math.random()*10))*place;
			place *= 10;
			index = (int)(Math.random()*al.length());
			value += al.substring(index, index+1);
		}
